...
function sortBySQL(field){
	...
	grid = $.fn.DtGrid.init({
		...
		check: boo,
		ajaxLoad : true,
		loadURL : sessionStorage.getItem("url")+'&searchText='+unicode($("#searchText").val())+'&sortFiled='+columnId,
		exportFileName : '数据列表',
	...
	grid.parameters["sortFiled"] = field;
	grid.parameters["sortInt"] = sessionStorage.getItem("sortInt");
	if(sessionStorage.getItem("sortInt") == 1){
		sessionStorage.setItem("sortInt",2);
	}else if(sessionStorage.getItem("sortInt") == 2){
		sessionStorage.setItem("sortInt",0);
	}else{
	...
	$.fn.DtGrid = {
		...
		init : function(options, name) {
			...
			var dtGridObject = {
				 ...
				 */
				//构建表格方法
				load : function(/*function*/callback /*(exhibitDatas)*/) {
					//定义表格对象映像
					...
					if(dtGridReflectionObj.init.toolsIsInit==false){
						...
						if(!dtGridReflectionObj.option.ajaxLoad){
							dtGridReflectionObj.hideProcessBar(function(){
								...
								if (callback) {
									callback(dtGridReflectionObj.exhibitDatas);
								}
							});
							return;
						}else{
					...
					if(!dtGridReflectionObj.option.ajaxLoad||dtGridReflectionObj.option.loadAll){
						...
						$.ajax({
							...
							success:function(pager){
								...
								dtGridReflectionObj.hideProcessBar(function(){
									if(!pager.isSuccess){
										$.fn.DtGrid.tools.toast(pager.message, 'warning', 5000);
										//构建表格、工具条
										dtGridReflectionObj.constructGrid();
										//加载工具按钮
										dtGridReflectionObj.constructGridToolBar();
				...
				constructGrid : function(){
					...
					////console.log(dtGridReflectionObj.option.id);
					//构件表头
					var gridContent = '';
					...
					if(dtGridReflectionObj.option.showHeader!=false){
						...
						for(var i=0; i<columns.length; i++){
							...
							gridContent += '	<th columnNo="'+i+'" columnId="'+columns[i].id+'" class="dt-grid-header '+dtGridReflectionObj.getColumnClassForHide(columns[i])+' '+columns[i].headerClass+' " style="'+columns[i].headerStyle+'">';
							}
					...
					if(dtGridReflectionObj.exhibitDatas!=null){
						...
						for (var i=0; i<dtGridReflectionObj.exhibitDatas.length; i++) {
							var entity = dtGridReflectionObj.exhibitDatas[i];
							...
							if (dtGridReflectionObj.option.check) {
								if (entity['checked'] == 'true') {
									gridContent += '	<td class="check-column text-center"><input type="checkbox" name="dt-grid-check" dataNo="'+i+'" id="dt_grid_'+dtGridReflectionObj.option.id+'_check_'+i+'" class="dt-grid-check" value="'+i+'" checked></td>';
								} else {
									gridContent += '	<td class="check-column text-center"><input type="checkbox" style="" name="dt-grid-check" dataNo="'+i+'" id="dt_grid_'+dtGridReflectionObj.option.id+'_check_'+i+'" class="dt-grid-check" value="'+i+'"></td>';
								}
							}
							var columns = dtGridReflectionObj.option.columns;
...
//						dtGridReflectionObj.reload();
				...
				constructGridPageBar : function(){
					...
					if(dtGridReflectionObj.exhibitDatas==null||dtGridReflectionObj.exhibitDatas.length<=0){
						...
						gridStatus.className = 'dt-grid-pager-status';
						...
						if(typeof dtGridReflectionObj.option.pageSizeLimit == 'object'){
							pageSizeElement += '<select id="dt_grid_change_page_size_'+dtGridReflectionObj.option.id+'" name="dt_grid_change_page_size_'+dtGridReflectionObj.option.id+'" class="change-page-size">';
							//整合可用页码
				...
				faseQuery : function(){
					...
					if(dtGridReflectionObj.init.fastQueryWindowIsInit){
						...
						var modal = $('#faseQuery_'+dtGridReflectionObj.option.id+'Modal');
						modal.css("opacity", "1");
						modal.css("z-index", "1200");
						modal.css("overflow", "scroll");
						modal.modal({});
						modal.css("margin-top", "1px");
						modal.css("padding-top", "430px");
						return;
					}
					var content = '';
					...
					$('#doFaseQuery_'+dtGridReflectionObj.option.id).click(function(){
						...
						$('#faseQuery_'+dtGridReflectionObj.option.id+'Modal').modal('close');
					});
				...
				advanceQuery : function(){
					...
					if(dtGridReflectionObj.init.advanceQueryWindowIsInit){
						...
						var modal = $('#advanceQuery_'+dtGridReflectionObj.option.id+'Modal');
						modal.css("opacity", "1");
						modal.css("z-index", "1200");
						modal.css("overflow", "scroll");
						modal.modal({});
						modal.css("margin-top", "1px");
						modal.css("padding-top", "300px");
						return;
					}
					//加载查询内容
					var content = '';
...
//					content += '		<li><a href="#advance_query_plan_'+dtGridReflectionObj.option.id+'" role="tab" data-toggle="tab">'+$.fn.DtGrid.lang[dtGridReflectionObj.option.lang].advanceQuery.plan.title+'</a></li>';
					content += '		<li id="advance_query_condition_'+dtGridReflectionObj.option.id+'" class="active"><a role="tab" data-toggle="tab">'+$.fn.DtGrid.lang[dtGridReflectionObj.option.lang].advanceQuery.condition.title+'</a></li>';
//					content += '		<li id="advance_query_sort_'+dtGridReflectionObj.option.id+'" ><a role="tab" data-toggle="tab">'+$.fn.DtGrid.lang[dtGridReflectionObj.option.lang].advanceQuery.sort.title+'</a></li>';
					content += '	</ul>';
...
//					});
				...
				doAdvanceQuery : function(){
					...
					$('#sortTable_'+dtGridReflectionObj.option.id+' tr[id*=tr_'+dtGridReflectionObj.option.id+'_]').each(function(){
						//console.log('#sortTable_'+dtGridReflectionObj.option.id+' tr[id*=tr_'+dtGridReflectionObj.option.id+'_]');
						var id = this.id.split('_')[2];
					...
					if(pass){
						...
						dtGridReflectionObj.load();
						$('#advanceQuery_'+dtGridReflectionObj.option.id+'Modal').modal('close');
					}
				},
					...
					for(var i=0; i<originalDatas.length; i++){
						...
						if(advanceQueryParameter.advanceQueryConditions&&advanceQueryParameter.advanceQueryConditions.length>0){
							...
							for(var j=0; j<advanceQueryParameter.advanceQueryConditions.length; j++){
								...
								if(advanceQueryCondition.logic=='asc'){
									condition += '&&';
								}
								if(advanceQueryCondition.logic=='desc'){
									condition += '||';
								}
							}
							try{
						...
						returnDatas = returnDatas.sort(function(record1, record2){
							for(var i=0; i<advanceQueryParameter.advanceQuerySorts.length; i++){
								...
								if(value1&&value2){
									...
									//数值比较
									if(!isNaN(value1)&&!isNaN(value2)){
										if(advanceQuerySort.logic=='asc'){
											return value1-value2;
										}
										if(advanceQuerySort.logic=='desc'){
											return value2-value1;
										}
									}
									//日期比较
				...
				insertConditionTr : function(){
					...
					content += '			<option value=""></option>';
					content += '			<option value="asc">and</option>';
					content += '			<option value="desc">or</option>';
					content += '		</select>';
					content += '	</td>';
					content += '</tr>';
				...
				insertSortTr : function(){
					...
					content += '			<option value=""></option>';
					content += '			<option value="asc">'+$.fn.DtGrid.lang[dtGridReflectionObj.option.lang].advanceQuery.sort.logic.asc+'</option>';
					content += '			<option value="desc">'+$.fn.DtGrid.lang[dtGridReflectionObj.option.lang].advanceQuery.sort.logic.desc+'</option>';
					content += '		</select>';
					content += '	</td>';
					content += '</tr>';
				...
				print : function(){
					...
					if(dtGridReflectionObj.init.printWindowIsInit){
						...
						var modal = $('#dtGridPrint_'+dtGridReflectionObj.option.id+'Modal');
						modal.css("opacity", "1");
						modal.css("z-index", "1200");
						modal.css("overflow", "scroll");
						modal.modal({});
					...
					$('#dt_grid_print_execute_'+dtGridReflectionObj.option.id).click(function(){
						...
						$('body').show();
						$('#dtGridPrint_'+dtGridReflectionObj.option.id+'Modal').modal('close');
					...
					$('#dt_grid_print_check_'+'_'+dtGridReflectionObj.option.id).bind("click", function() {
						var val = $('#dt_grid_export_check_'+'_'+dtGridReflectionObj.option.id).prop("checked");
						////console.log("val:"+val)
						$('input[id*=dt_grid_print_check_'+'_'+dtGridReflectionObj.option.id+'_]').each(function(){
				...
				exportFile : function(exportType){
					...
					if(dtGridReflectionObj.init.exportWindowIsInit[exportType]){
						...
						var modal = $('#dtGridExport_'+exportType+'_'+dtGridReflectionObj.option.id+'Modal');
						modal.css("opacity", "1");
						modal.css("z-index", "1200");
						modal.css("overflow", "scroll");
						modal.modal({});
					...
					$('#dt_grid_export_execute_'+exportType+'_'+dtGridReflectionObj.option.id).click(function(){
						...
						dtGridPagerInput.id = 'pager';
						dtGridPagerInput.name = 'pager';
						dtGridPagerInput.value = JSON.stringify(dtGridPager);
						
						////console.log($.param(dtGridPager, false))
						//dtGridPagerInput.value = $.param(dtGridPager)
						...
						$('#dtGridExport_'+exportType+'_'+dtGridReflectionObj.option.id+'Modal').modal('close');
					});
					...
					$('#dt_grid_export_check_'+exportType+'_'+dtGridReflectionObj.option.id).bind("click", function() {
						var val = $('#dt_grid_export_check_'+exportType+'_'+dtGridReflectionObj.option.id).prop("checked");
						////console.log("val:"+val)
						$('input[id*=dt_grid_export_check_'+exportType+'_'+dtGridReflectionObj.option.id+'_]').each(function(){
							 $(this).prop("checked", val);
						});
					});
				...
				getExhibitDatas: function() {
					//设置本体映射
					dtGridReflectionObj = this;
					return dtGridReflectionObj.exhibitDatas;
				},
...
(function($) {
	$.fn.DtGrid.defaultOptions = {
		grid : {
			...
			exportFileName : 'datas',
			exportURL : 'dtgridexport.do?p=export',/*jiangtao changed 20150427*/
			pageSize : 20,
