	...
	<div id="page" style='top:${model.direct==1?0:'60px'}'>
		...
		<div class="admin-content am-animation-slide-right clearfix">
			<div style="padding:20px; ">
				<h2 style="color:#353535; display:inline;">派出所管理系统» 查看派出所</h2>
			...
			<div class="panel-body">
				<form id="mainForm" class="form-horizontal" role="form" method='post'>
					...
					<div class="form-group">
						<label class="col-md-2 control-label" for="userID">标题:</label>
						<input class="hidden" name="dateID" id="dateID" value="${model.entity.dateID}">
						<input class="hidden" name=url" id="url" value="${model.entity.url}">
						<div class="col-md-7">                                                                             
							<input type="text" name="title" id="title" placeholder="" value="${model.entity.title}"  class="form-control" >
						</div>	
							</div>			
					<div class="form-group">
						<label class="col-md-2 control-label" for="userID">说明:</label>
						<div class="col-md-7">                                                                             
							
							<input type="text" name="explain" id="explain" placeholder="" value="${model.entity.explain}"  class="form-control" >
						</div>	
					</div>
...
// 	});
	...
	$('#loading-waiting-modal').modal({closeViaDimmer:false});
		$.ajax({
			...
			if(data.error==null)
				handsuc(data, 'update', 'date.do?p=view&parentID=12323&moduleID=5678');
			else
				handerr(data);	
			},
	...
	$('#pageCloseBtn').click(function() {
		window.location.href = "date.do?p=view&parentID=12323&moduleID=5678";
	});
</script>
<script>
